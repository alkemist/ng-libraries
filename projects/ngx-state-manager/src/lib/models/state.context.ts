import { AnyValue, ValueKey, ValueRecord } from "@alkemist/smart-tools";
...
export class StateContext<S extends ValueRecord> {
  ...
  setItem<T>(paths: ValueKey[] | ValueKey, item: T) {
    const items = this.getItems<T>(paths);

    const findResult = this.state.findRightItem(paths, item as AnyValue);

    if (findResult.index > -1) {
      items[findResult.index] = item;
    }

    return this.setItems(paths, items);
  }

  setItemWithSelector<T>(paths: ValueKey[] | ValueKey, selector: (item: T) => boolean, item: T) {
    const _items = this.getItems<T>(paths);
    const items = _items.map(_item => selector(_item) ? item : _item);
    return this.setItems(paths, items);
  }

  patchItem<T>(paths: ValueKey[] | ValueKey, item: Partial<T>) {
    const items = this.getItems<T>(paths);

    const findResult = this.state.findRightItem(paths, item as AnyValue);

    if (findResult.index > -1) {
      items[findResult.index] = {
        ...findResult.value,
        ...item
      } as T;
    }
  ...
  patchItemWithSelector<T>(paths: ValueKey[] | ValueKey, selector: (item: T) => boolean, item: Partial<T>) {
    const _items = this.getItems<T>(paths);
  ...
  removeItem<T>(paths: ValueKey[] | ValueKey, item: T) {
    const items = this.getItems<T>(paths);

    const findResult = this.state.findRightItem(paths, item as AnyValue);

    if (findResult.index > -1) {
      items.splice(findResult.index, 1);
    }

    return this.setItems(paths, items);
  }

  removeItemWithSelector<T>(paths: ValueKey[] | ValueKey, selector: (item: T) => boolean) {
    const _items = this.getItems<T>(paths);
